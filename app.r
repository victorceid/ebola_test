    ...
    ui <- pageWithSidebar(
        ...
        mainPanel(
          ...
          tabPanel("Model Description",
              ...
              helpText("The model is expressed as a system of coupled ordinary differential equations (ODE) with periodic disease transmission and dispersal between the PATCH A, an endemic enviroment for the virus, e.g. intact forest, and the PATCH B, an human modified enviromental, e.g. village or human settlement."),
              helpText("The total population size in each patch for j = A, B, is: $$Nj = Sj +Ij +Rj$$  The total population size is: $$N = NA +NB$$ For the current model: $$NA = 100$$ $$NB = 1000$$"),
              helpText("Each population patch is partitioned into Susceptible (S), Infected (I), and Recovered (R) sub-populations. $$\\theta$$ "))
          ))
        )
    ),
